# Kaggle one-cell runner: reviewer sensitivity matrix, Session 10 of 14.
...
def valid_repo(path: Path) -> bool:
    return (
        (path / "tools" / "run_reviewer_revision.py").is_file()
        and (path / "kaggle" / "reviewer_sensitivity" / "sensitivity_protocol.py").is_file()
        and (path / "kaggle" / "reviewer_sensitivity" / "sensitivity_session_runner.py").is_file()
    )
...
def clone_public_repo() -> Path:
    ...
    if not valid_repo(TARGET):
        raise RuntimeError("Cloned repo does not contain the corrected reviewer-sensitivity Kaggle workflow. Push this corrected package first.")
...
run([
    sys.executable,
    REPO / "kaggle" / "reviewer_sensitivity" / "sensitivity_session_runner.py",
