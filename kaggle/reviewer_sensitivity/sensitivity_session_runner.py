...
from sensitivity_protocol import (  # noqa: E402
...
def find_resume_zip(session_id: int) -> Path | None:
    ...
    if not base.exists():
        return None
    pattern = f"*reviewer*sensitivity*session*{session_id:02d}*results*.zip"
...
def print_plan(session_id: int, gpu_name: str, factor: float) -> None:
    print("=" * 104)
    print(f"Reviewer sensitivity session {session_id:02d}/{SESSION_COUNT} | GPU: {gpu_name} | initial T4 factor: {factor:.2f}")
...
def main() -> int:
    ...
    output = work / f"trso_reviewer_sensitivity_session_{session_id:02d}_results"
